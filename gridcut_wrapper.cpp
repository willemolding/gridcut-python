                                           ...
                                           PyObject *keywds)
{
	...
	//parse arguments
	static char *kwlist[] = {"width", "height", "pairwise_cost",
	                        "source", "sink", "n_threads", "block_size", NULL};

	if (!PyArg_ParseTupleAndKeywords(args, keywds, "iifOO|ii", kwlist,
	    &width, &height, &w, &source, &sink, &n_threads, &block_size)) return NULL;
...
static PyObject* gridcut_solve_2D_4C(PyObject* self, PyObject *args, PyObject *keywds)
{
    ...
    if (source_arr == NULL || sink_arr == NULL || up_arr==NULL || down_arr==NULL
        || left_arr == NULL || right_arr == NULL)
        return NULL;
...
static char gridcut_docs[] =
    "Any message you want to put here...\n";
